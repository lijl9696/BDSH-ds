...
| superset | `apache/superset:latest` | `${TG_REPORT_DATA_DIR}/superset/home` |
| superset-postgres | `postgres:16` | `${TG_REPORT_DATA_DIR}/superset/postgres` |
| backup | `postgres:16` | `${TG_REPORT_DATA_DIR}/backups` |

只要不删除 `${TG_REPORT_DATA_DIR}`，更新 `report-web` 镜像不会影响业务数据、BI 仪表盘、登录账号和上传文件。
...
METABASE_PORT=3000
SUPERSET_PORT=8088
DASHBOARD_URL=http://群晖IP:3000
POSTGRES_PORT=5729
SUPERSET_IMAGE=apache/superset:latest
SUPERSET_SECRET_KEY=换成一个足够长的随机密钥
SUPERSET_POSTGRES_PASSWORD=换成一个足够长的 Superset 元数据库密码
SUPERSET_ADMIN_USERNAME=admin
SUPERSET_ADMIN_PASSWORD=换成一个 Superset 管理员密码
...
mkdir -p /volume1/docker/tg-report/superset/postgres
mkdir -p /volume1/docker/tg-report/superset/home
...
Superset 用作新的 BI 工具时可以和 Metabase 并行运行。Superset 自身需要一个独立 PostgreSQL 保存用户、数据集、图表和仪表盘等元数据；业务明细数据仍然保存在本系统的 `tg-report-postgres`。

启动 Superset：

```bash
cd /volume1/docker/BDSH-ds/data-platform-system/deploy
docker-compose -f docker-compose.synology.yml -f docker-compose.superset.yml pull superset-postgres superset
docker-compose -f docker-compose.synology.yml -f docker-compose.superset.yml up -d superset-postgres superset
```

如果拉镜像慢，可以先在当前 SSH 终端临时启用代理后再执行 `pull`：
...
```text
Superset: http://群晖IP:8088
默认账号: .env 里的 SUPERSET_ADMIN_USERNAME
默认密码: .env 里的 SUPERSET_ADMIN_PASSWORD
```

首次登录后建议立刻修改默认密码。

在 Superset 新增 PostgreSQL 数据源时，连接本系统业务库填写：
...
```text
postgresql+psycopg2://tg_report:你的POSTGRES_PASSWORD@postgres:5432/tg_report
```

因为 Superset 与 `postgres` 在同一个 Docker Compose 网络里，主机不要填群晖 IP，也不要填外部映射端口 `5729`。

确认 Superset 可用后，如需把前台“打开仪表盘”按钮切到 Superset，把 `.env` 改为：

```text
DASHBOARD_URL=http://群晖IP:8088
```
...
新版 collector 会校验下载文件名里的日期范围。比如 2026-06-12 执行时，目标文件必须包含 `20260611-20260611`；如果下载到 `20260610-20260610`，不会入库，会等待后重试。第一轮会使用“昨天”快捷项；如果日期不匹配，后续重试会绕过“昨天”快捷项，直接在日期面板里点击目标日期两次。
