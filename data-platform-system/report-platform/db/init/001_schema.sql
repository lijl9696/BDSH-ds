...
CREATE TABLE IF NOT EXISTS area_assignments (
    ...
    owner VARCHAR(128) NOT NULL,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uq_area_assignment_province_city_store UNIQUE(province, city, store_name)
);
...
CREATE INDEX IF NOT EXISTS idx_area_assignments_lookup ON area_assignments(province, city, store_name);
