...
4. 门店主数据维护在 `stores`。
5. 区域/负责人归属优先读取 `area_assignments`：先按省份 + 城市 + 门店名称匹配，再按省份 + 城市兜底。
...
1. 如果配置行有 `store_name`，报表导入时优先按 `province + city + store_name` 匹配。
2. 如果没有命中门店级配置，再按 `province + city + store_name=''` 匹配城市兜底配置。
3. 同一省市可以同时存在一条城市兜底配置和多条门店级配置。
4. 匹配时会做标准化：省市会去掉末尾行政后缀，如 `河南省` 和 `河南` 视为同一值；门店名会统一中英文括号和空格，如 `彭世修脚(龙子湖直营店)` 和 `彭世修脚（龙子湖直营店）` 视为同一值。
...
| 名称 | 字段 | 说明 |
| --- | --- | --- |
| uq_area_assignment_province_city_store | province, city, store_name | 同一省市同一门店只能有一条配置 |
| idx_area_assignments_lookup | province, city, store_name | 导入报表时快速匹配归属 |
...
-- 查看某城市的城市兜底和门店级配置
SELECT province, city, store_name, region, owner
...
- `区域配置` 可选字段：`门店名称`。填写后表示门店级覆盖；留空表示城市兜底。
- 人工排查门店归属时，注意源报表和配置表可能存在省份后缀、中英文括号、空格差异；应用匹配时会标准化，但直接 SQL 精确查询时仍要留意原始写法。
- 如需重新上传区域配置，可先清空 `area_assignments`，不需要清空 `stores` 或指标明细表。
