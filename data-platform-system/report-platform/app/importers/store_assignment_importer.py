...
def import_store_assignments(db: Session, path: Path) -> dict[str, int]:
    ...
    area_columns = _resolve_columns(area_df, AREA_COLUMNS, AREA_SHEET, optional={"store_name"})
...
def _import_areas(db: Session, df: pd.DataFrame, columns: dict[str, str]) -> tuple[dict[tuple[str, str, str], tuple[str, str]], dict[str, int]]:
    ...
    for _, row in df.iterrows():
        ...
        if not province or not city or not region or not owner:
            warnings += 1
            continue
        lookup[assignment_key(province, city, store_name)] = (region, owner)
    upserted = 0
    for key, (region, owner) in lookup.items():
        ...
        existing = next(
            (
                ...
                if assignment_key(item.province, item.city, item.store_name) == key
            ),
            None,
        )
        if existing:
            ...
            province, city, store_name = key
            db.add(AreaAssignment(province=province, city=city, store_name=store_name, region=region, owner=owner))
        upserted += 1
...
def _import_stores(
    ...
    area_lookup: dict[tuple[str, str, str], tuple[str, str]],
) -> dict[str, int]:
    upserted = 0
    updated_stores = 0
    warnings = 0
    for _, row in df.iterrows():
        ...
        province = clean_text(row.get(columns["province"]))
        city = clean_text(row.get(columns["city"]))
        area = area_lookup.get(assignment_key(province, city, store_name)) or area_lookup.get(assignment_key(province, city, ""))
...
def sync_stores_from_area_assignments(db: Session) -> int:
    assignments = db.query(AreaAssignment).all()
    if not assignments:
        return 0
    store_assignments = {
        assignment_key(item.province, item.city, item.store_name): (item.region, item.owner)
        for item in assignments
        if normalize_store_name(item.store_name)
    }
    city_assignments = {
        assignment_key(item.province, item.city, ""): (item.region, item.owner)
        for item in assignments
    ...
    for store in db.query(Store).all():
        if not store.province or not store.city:
            continue
        area = store_assignments.get(assignment_key(store.province, store.city, store.name))
        if not area:
            area = city_assignments.get(assignment_key(store.province, store.city, ""))
        if not area:
            continue
        region, owner = area
        if store.region != region or store.owner != owner:
            store.region = region
            store.owner = owner
            updated += 1
    return updated
...
def _ensure_area_assignment_schema(db: Session) -> None:
    ...
    db.execute(
        text(
            "DO $$ BEGIN "
            "IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_area_assignment_province_city_store') THEN "
            "ALTER TABLE area_assignments ADD CONSTRAINT uq_area_assignment_province_city_store UNIQUE (province, city, store_name); "
            "END IF; "
            "END $$;"
        )
    )
